...
public class MenuBarBuilder extends IBuilder<GMenuBar> implements Buildable {
    ...
    private final KaroDesigner karoDesigner;
    private GMenuBar menuBar;

    public MenuBarBuilder(KaroDesigner karoDesigner) {
    ...
    private void buildMenu02(GMenuBar menuBar) {
        ...
        JMenuItem item01 = new GMenuItem("Paint-View x", true) {
            ...
            public void performAction(ActionEvent actionEvent) {
                ...
                if (isActivated()) {
                    karoDesigner.getPaintPanel().off();
                    setText("Paint-View");
                } else {
                    karoDesigner.getPaintPanel().on();
    ...
    private void buildMenu03(GMenuBar menuBar) {
        GMenu menu03 = new GMenu("Models");
        for (Model model : karoDesigner.getModels()) {
            menu03.add(new JMenuItem(model.toString()));
        }

        menu03.add(ItemDivider.getDefaultItemDivider());
        menuBar.add(menu03);
    }
